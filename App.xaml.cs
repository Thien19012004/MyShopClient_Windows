...
public partial class App : Application
{
 ...
 TaskScheduler.UnobservedTaskException += TaskScheduler_UnobservedTaskException;
    ...
    // 1. Đăng ký ServerConfigService trước
        ...
        // Connection error handler - để xử lý lỗi connection toàn cục
   services.AddSingleton<IConnectionErrorHandler, ConnectionErrorHandler>();
...
// Register GraphQL client abstraction (uses the named "Api" client internally)
 ...
 services.AddTransient<CustomerListViewModel>();
  ...
  Services = services.BuildServiceProvider();
        ...
        // Subscribe to connection error events to navigate to login page
SetupConnectionErrorHandler();
    }

    /// <summary>
    /// Setup global connection error handler to navigate to login when server is unreachable
    /// </summary>
    private void SetupConnectionErrorHandler()
    {
        var connectionErrorHandler = Services.GetRequiredService<IConnectionErrorHandler>();
        var navigationService = Services.GetRequiredService<INavigationService>();
        var authService = Services.GetRequiredService<IAuthService>();

   connectionErrorHandler.ConnectionErrorOccurred += async (sender, args) =>
        {
   Debug.WriteLine($"[App] Connection error detected: {args.Message}");
 ...
 try
 {
         // Logout to clear any cached credentials
        await authService.LogoutAsync();
   
  // Navigate to login page
      navigationService.NavigateToLogin();
 
 // Show error message to user (optional - could show a dialog)
     Debug.WriteLine($"[App] Navigated to login due to connection error");
      }
  catch (Exception ex)
   {
 Debug.WriteLine($"[App] Error handling connection error: {ex.Message}");
                }
    });
   }
        };
